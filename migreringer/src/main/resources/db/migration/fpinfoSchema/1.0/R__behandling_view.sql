...
AS SELECT
  ...
  FROM
    fpsak.BEHANDLING beh
    INNER JOIN fpsak.FAGSAK fs ON beh.FAGSAK_ID = fs.id
    LEFT OUTER JOIN fpsak.BEHANDLING_RESULTAT br ON beh.id = br.BEHANDLING_ID
    LEFT OUTER JOIN fpsak.GR_FAMILIE_HENDELSE gfh ON beh.id = gfh.BEHANDLING_ID
    LEFT OUTER JOIN fpsak.FH_FAMILIE_HENDELSE fh ON fh.id = NVL(gfh.OVERSTYRT_FAMILIE_HENDELSE_ID, NVL(gfh.BEKREFTET_FAMILIE_HENDELSE_ID, gfh.SOEKNAD_FAMILIE_HENDELSE_ID))
    LEFT OUTER JOIN fpsak.BEHANDLING_ARSAK ba ON beh.id = ba.BEHANDLING_ID
  WHERE
